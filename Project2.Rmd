...
#Run model
tele.knn.preds <- ifelse(knn(tele.knn.train, tele.knn.test, cl = tele.train$y, k = 10) == "yes", 1, 0)
```
...
tele.ann.preds <- ifelse(predict(nn.model, tele.ann.test) > .3, 1, 0)
```
...
logreg.preds <- ifelse(predict(tele.logreg2, tele.test, type = "response") > .3, 1, 0)
```
